...
namespace ShearCell_Interaction.Presenter
{
    public class MetamaterialPresenter
    {
        ...
        public MetamaterialModel Model
        {
        ...
        public void TraceInputAndOutput()
        {
            if (Model.InputVertex != null)
            {
                _viewModel.AddTracingPoint(Model.InputVertex);
            }

            if (Model.OutputVertex != null)
            {
                _viewModel.AddTracingPoint(Model.OutputVertex);
            }
        }
        ...
        internal void ExportConfiguration(MetamaterialModel model, string filename = null)
        {
            var filepath = filename;
        ...
        {
            _viewModel.BruteForceAnchors = _viewModel.Model.GetAnchors().Count < 1;

            _hierarchicalOptimizationController.FilePrefix = DateTime.Now.ToString("yyyy-dd-M--HH-mm-ss");
        ...
        public void SetTracedOutputAsOutput()
        {
            var tracedPoints = _viewModel.TracedVertices;
            var tracedOutputLine = tracedPoints[_viewModel.Model.OutputVertex];
            var tracedOutputPoints = tracedOutputLine.Line.Points.Select(p => CoordinateConverter.ConvertScreenToGlobalCellCoordinates(new Vector(p.X, p.Y))).ToList();

            _viewModel.OutputPath = tracedOutputLine.Line.Points;
            _viewModel.Model.OutputPath = tracedOutputPoints;
        }
    }
}
