...
namespace ShearCell_Interaction.Model
{
    public class SimulationModel
    {
        ...
        public int MinTargetDof = 2;
        public int MaxTargetDof = 5;
        public static double MinErrorDelta = .001;
        public static double MinImprovement = .05;
        ...
        //result improved if error is smaller than MinErrorDelta or slightly larger (-MinErrorDelta) but smaller DoF
        public static bool ResultDidImprove(double currentMinError, double pastMinError, int currentDof, int pastDof)
        {
            var errorDifference = pastMinError - currentMinError;
            var doFDifference = pastDof - currentDof;
            var resultDidImprove = errorDifference > SimulationModel.MinImprovement || (errorDifference < SimulationModel.MinImprovement && errorDifference > -SimulationModel.MinImprovement && doFDifference > 0);

            return resultDidImprove;
