...
namespace ShearCell_Interaction.Model
{
    public class MetamaterialModel
    {
        ...
        public void Scale(int scaleFactor, bool scalePaths = false)
        {
            ...
            Vertex minAnchor = anchors[0];
            var minAnchorToOriginDistance = minAnchor.ToVector().X + minAnchor.ToVector().Y;
            
            foreach(var anchor in anchors)
            {
                var anchorDistance = anchor.ToVector().X + anchor.ToVector().Y;
                if (anchorDistance < minAnchorToOriginDistance)
                    minAnchor = anchor;
            }

            var scaledMinAnchorPosition = minAnchor.ToVector() * scaleFactor;
            AddAnchor(scaledMinAnchorPosition);

            foreach (var anchor in anchors)
            {
                if (anchor != minAnchor)
                {
                    var preScaleDistanceToMinAnchor = anchor.ToVector() - minAnchor.ToVector();
                    var newAnchorPosition = scaledMinAnchorPosition + preScaleDistanceToMinAnchor;
                    AddAnchor(newAnchorPosition);
                }
            }
        ...
        public void ScalePath(double scaleFactor)
        {
        ...
        public string GetEncoding()
        {
            ...
            var width = Vertices.Max(v => v.InitialX) - Vertices.Min(v => v.InitialX);
