...
namespace ShearCell_Interaction.Optimisation
{
    ...
    public class IterativeCellOptimizationController
    {
        ...
        private int _successiveRejectedSolutionCounter;
        private const int MAX_NUM_SUCC_REJ_SOLUTIONS = 200;

        private int _successiveErrorSolutionCounter;
        private const int MAX_NUM_SUCC_ERROR_SOLUTIONS = 30;
        ...
        private void OnSimulationInfoAvailable(object sender, SimulationInfoEventArgs e)
        {
            ...
            if (solutionIsRigid)
            {
                _successiveErrorSolutionCounter++;
                ...
                if (_successiveErrorSolutionCounter > MAX_NUM_SUCC_ERROR_SOLUTIONS)
                {
                    Console.WriteLine($@"Stuck in local minimum for {_successiveErrorSolutionCounter} iterations. Finishing up.");
            ...
            _successiveErrorSolutionCounter = 0;
            ...
            bool accepted = SimulationModel.ResultDidImprove(_simModel.CurrentError, _simModel.MinError, _simModel.CurrentDof, _simModel.Model.CurrentDoF ) ;
            ...
            if (accepted)
            {
                ...
                StoreAsBestResult();
#if (VERBOSE)
                Console.WriteLine($@"Iteration {_simModel.CurrentNumIterations}/{_simModel.MaxNumIterations}. Solution accepted with error {_simModel.MinError.ToString("F4")}");
#endif
                _simModel.NumAcceptedSolutions++;
                _successiveRejectedSolutionCounter = 0;
            }
            else if (forceChange)
            {
                _simModel.MinError = _simModel.CurrentError;
#if (VERBOSE)
                Console.WriteLine($@"Iteration {_simModel.CurrentNumIterations}/{_simModel.MaxNumIterations}. Solution accepted (SimAnn {currentTemp.ToString("F3")}, {objective.ToString("F3")}) with error {_simModel.MinError.ToString("F4")}");
#endif
                _simModel.Model.UpdateConstraintsGraph();
            ...
            {
#if (VERBOSE)
                Console.WriteLine($@"Iteration {_simModel.CurrentNumIterations}/{_simModel.MaxNumIterations}. Solution rejected with error {_simModel.CurrentError.ToString("F4")}");
#endif
                ResetSolution(_cellStorage);

                _simModel.Model.UpdateConstraintsGraph();
                _simModel.NumRejectedSolutions++;
                _successiveRejectedSolutionCounter++;
            }
            ...
            if (_simModel.CurrentNumIterations >= _simModel.MaxNumIterations)
            {
                _simulationController.OnSimulationInfoAvailable -= OnSimulationInfoAvailable;
                CompleteOptimization();
            }else if (_successiveRejectedSolutionCounter >= MAX_NUM_SUCC_REJ_SOLUTIONS)
            {
                Console.WriteLine(@"MAX NUM SUCC REJ SOLUTIONS REACHED");
                _simulationController.OnSimulationInfoAvailable -= OnSimulationInfoAvailable;
                CompleteOptimization();
            }
            else
        ...
        private double GetOutputError(List<List<Vector>> framePositions)
        {
            if (_simModel.Model.OutputVertex != null)
            {
                ...
                if (outputIndex >= 0)
                {
                    ...
                    if (MathHelper.IsEqualDouble(length, 0.0))
                        return -1;
        ...
        {
            bool shouldRestore = SimulationModel.ResultDidImprove(_simModel.CurrentError, bestResultError, _simModel.CurrentDof, bestResultDof); 
